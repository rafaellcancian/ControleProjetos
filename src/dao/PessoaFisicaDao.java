...
public class PessoaFisicaDao implements ClienteDao {
    ...
    public int inserir(Cliente cliente) {
        ...
        Object parCliente[] = {pessoaFisica.getEndereco(), pessoaFisica.getEmail(), pessoaFisica.getFone()};
    ...
    public int alterar(Cliente cliente) {
        PessoaFisica pessoaFisica = (PessoaFisica) cliente;
        Object parCliente[] = {pessoaFisica.getEndereco(), pessoaFisica.getEmail(), pessoaFisica.getFone(), pessoaFisica.getIdCliente()};
        PreparedStatement pstmt = null;
    ...
    public List<PessoaFisica> getLista() {
        try {
            ...
            String sql1 = "SELECT * FROM Cliente INNER JOIN PessoaFisica "
                + "ON (idCliente = idPessoaFisica) ORDER BY Nome";
    ...
    public Cliente getClienteId(int id) {
        try {
            ...
            String sql1 = "SELECT * FROM Cliente INNER JOIN PessoaFisica "
                + "ON (idCliente = idPessoaFisica) WHERE idCliente=?";
            PreparedStatement pstmt = con.prepareStatement(sql1);
            pstmt.setInt(1, id);
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
    ...
    public List<PessoaFisica> getClienteNome(String nome) {
        try {
            List<PessoaFisica> lstPessoaFisica = new ArrayList<>();
            String sql1 = "SELECT * FROM Cliente INNER JOIN PessoaFisica Pf "
                + "ON (idCliente = idPessoaFisica) WHERE Pf.nome LIKE ?";
            PreparedStatement pstmt = con.prepareStatement(sql1);
    ...
    public List<PessoaFisica> getClienteEmail(String email) {
        try {
            List<PessoaFisica> lstPessoaFisica = new ArrayList<>();
            String sql1 = "SELECT * FROM Cliente INNER JOIN PessoaFisica "
                + "ON (idCliente = idPessoaFisica) WHERE email LIKE ?";
            PreparedStatement pstmt = con.prepareStatement(sql1);
    ...
    public List<PessoaFisica> getClienteFone(String fone) {
        try {
            List<PessoaFisica> lstPessoaFisica = new ArrayList<>();
            String sql1 = "SELECT * FROM Cliente INNER JOIN PessoaFisica "
                + "ON (idCliente = idPessoaFisica) WHERE fone LIKE ?";
            PreparedStatement pstmt = con.prepareStatement(sql1);
